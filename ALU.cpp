...
ALUOperation ALUController::getALUOperation(ALUOp aluOp, InstructionParts parts) {
    if (aluOp == ALU_OP_ADD) {
        ...
        // Check opcode to distinguish between R-type and I-type
        uint8_t opcode = parts.opcode;
        if (opcode == 0x33) { // R-type instructions
            switch (parts.funct3) {
                case 0x0: // SUB (R-type)
                    if (parts.funct7 == 0x20) {
                        ...
                        return ALU_INVALID; // No plain ADD in requirements
                    }
                case 0x7: // AND (R-type)
                    return ALU_AND;
            ...
            switch (parts.funct3) {
                case 0x0: // ADDI (I-type)
                    return ALU_ADD; // ADD with immediate
